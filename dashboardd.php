...
// Fetch selected years for each section
...
if ($role_id == 1 || $role_id == 2) { // Admin and Staff
    ...
    if ($role_id == 2) {
        ...
        $activeTrainersQuery = $conn->query("
            SELECT COUNT(*) AS active_count 
            FROM users 
            WHERE role_id = 3 AND status = 1 AND YEAR(created_at) = $trainerInsightYear
        ");
        $inactiveTrainersQuery = $conn->query("
            SELECT COUNT(*) AS inactive_count 
            FROM users 
            WHERE role_id = 3 AND status = 0 AND YEAR(created_at) = $trainerInsightYear
        ");
        ...
        // Course Status Data (For Staff)
        $courseStatusQuery = $conn->query("
            SELECT 
                SUM(CASE WHEN valid_to > CURDATE() AND status = 1 THEN 1 ELSE 0 END) AS upcoming,
                SUM(CASE WHEN valid_to BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN 1 ELSE 0 END) AS expiring,
                SUM(CASE WHEN valid_to < CURDATE() THEN 1 ELSE 0 END) AS expired,
                SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END) AS completed,
                SUM(CASE WHEN status = 0 THEN 1 ELSE 0 END) AS inactive
            FROM courses
            WHERE YEAR(created_at) = $courseStatusYear
        ");
            ...
            <div id="staff-dashboard">
                ...
                <div class="dashboard-row">
                    <div class="chart-card">
                        ...
                        <div class="filter-year">
                            <form method="GET" action="dashboard.php">
                                <label for="trainer-insight-year">Select Year:</label>
                                <select name="trainer-insight-year" id="trainer-insight-year" onchange="this.form.submit()">
                                    <?php foreach ($availableYears as $year): ?>
                                        <option value="<?php echo $year; ?>" <?php echo ($year == $trainerInsightYear) ? 'selected' : ''; ?>>
                                            <?php echo $year; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <div class="chart-card">
                        ...
                        <div class="filter-year">
                            <form method="GET" action="dashboard.php">
                                <label for="course-status-year">Select Year:</label>
                                <select name="course-status-year" id="course-status-year" onchange="this.form.submit()">
                                    <?php foreach ($availableYears as $year): ?>
                                        <option value="<?php echo $year; ?>" <?php echo ($year == $courseStatusYear) ? 'selected' : ''; ?>>
                                            <?php echo $year; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                                    ...
                                    <?php while ($activity = $recentActivities->fetch_assoc()): ?>
                                        <div class="planner-event">
                                            ...
                                            <div class="planner-details">
                                                ...
                                                <p class="time"><?php echo htmlspecialchars($activity['details']); ?></p>                                                </div>
                                            </div>
                                    <?php endwhile; ?>
                                <?php else: ?>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                        ...
                        const trainerStatusCtx = document.getElementById('trainerStatusChart').getContext('2d');
                            new Chart(trainerStatusCtx, {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        label: 'Trainer Count',
                                        data: [<?php echo $activeTrainers; ?>, <?php echo $inactiveTrainers; ?>],
                    ...
                    const courseStatusCtx = document.getElementById('courseStatusChart').getContext('2d');
                        new Chart(courseStatusCtx, {
                            type: 'pie',
                            data: {
                                labels: ['Upcoming', 'Expiring', 'Expired', 'Completed', 'Inactive'],
                                datasets: [{
                                    data: [
                                        <?php echo $courseStatusData['upcoming']; ?>,
                                        <?php echo $courseStatusData['expiring']; ?>,
                                        <?php echo $courseStatusData['expired']; ?>,
                                        <?php echo $courseStatusData['completed']; ?>,
                                        <?php echo $courseStatusData['inactive']; ?>
                                    ],
                    ...
                    new Chart(courseStatusCtx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                data: [
                                    <?php echo $conn->query("SELECT COUNT(*) FROM `courses` WHERE `valid_to` > CURDATE() AND `status` = 1")->fetch_row()[0]; ?>,
                                    <?php echo $conn->query("SELECT COUNT(*) FROM `courses` WHERE `valid_to` BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)")->fetch_row()[0]; ?>,
                                    <?php echo $conn->query("SELECT COUNT(*) FROM `courses` WHERE `valid_to` < CURDATE()")->fetch_row()[0]; ?>,
                                    <?php echo $conn->query("SELECT COUNT(*) FROM `courses` WHERE `status` = 1")->fetch_row()[0]; ?>,
                                    <?php echo $conn->query("SELECT COUNT(*) FROM `courses` WHERE `status` = 0")->fetch_row()[0]; ?>
                                ],
                                backgroundColor: ['#FFCE56', '#FF6384', '#36A2EB', '#4BC0C0', '#9966FF']
                            }]
                        },
                    ...
                    // Venue Heatmap
                    const venueData = <?php echo json_encode($venueUsageQuery->fetch_all(MYSQLI_ASSOC)); ?>;
